...
export default function RegisteredPropertyLeaseListing(props) {
  const classes = useStyles();
  const userDetail = useSelector(state=>state.userDetail)
  const [token2, changeToken] = useState("")
  const url = BASEURL+'/register-sales-leases/mine'
          ...
          <TableBody>
            {allNews.map(row => (
              <StyledTableRow key={row.id}>
                ...
                <StyledTableCell align="left">{row.amount_paid}</StyledTableCell>
                <StyledTableCell align="left">{row.type_of_property}</StyledTableCell>
