...
export default function RegisteredDirectoryListing(props) {
  const classes = useStyles();
  const userDetail = useSelector(state=>state.userDetail)
  const [token2, changeToken] = useState("")
  const url = BASEURL+'/tenancies/mine'
...
const makePayment = (entity_id) => {
  ...
  formData.append('entity_type', "tenancy")   
...
const notify = (message) => toast(message);
  ...
  <DashboardLayout width="100%" showCurvedFooter={false} showFooter={false} showBackgroundImage={false} backgroundColor="white" height="700px">
    <HeaderDescription title="Welcome to your property Registered board" description="This board contains all the property registered by you in the platform" />
          ...
          <TableBody>
            {allNews.map(row => (
              <StyledTableRow key={row.id}>
                <StyledTableCell align="left">{row.description_of_property}</StyledTableCell>
                ...
                <StyledTableCell align="left">{verifierPaid(row.paid, row.id)}</StyledTableCell>
